...
# Dead zone threshold
dead_zone_threshold = 10  # Adjust the dead zone threshold as needed

# Click gesture variables
click_threshold = 50  # Adjust the threshold for the click gesture
click_countdown = 0
...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        if time.time() - last_move_time > delay:
            ...
            # Apply dead zone to eliminate small, unintended movements
            if last_hand_position is not None and abs(x - last_hand_position[0]) < dead_zone_threshold \
                    and abs(y - last_hand_position[1]) < dead_zone_threshold:
                x = last_hand_position[0]
                y = last_hand_position[1]
            ...
            if hand_speed < click_threshold:
                if click_countdown == 0:
                    mouse.press(Button.left)
                    click_countdown = 5  # Set a countdown to prevent multiple clicks in quick succession
            else:
                mouse.release(Button.left)

            # Decrease the click countdown
            if click_countdown > 0:
                click_countdown -= 1
