...
while True:
    ...
    if results.multi_hand_landmarks:
        ...
        if time.time() - last_move_time > delay:
            ...
            x, y = apply_smoothing((x, y), last_hand_position, 0.2)

            # Check if the movement is within the dead-zone
            if (
                last_hand_position is not None and
                abs(x - last_hand_position[0]) > dead_zone_radius and
                abs(y - last_hand_position[1]) > dead_zone_radius
            ):
                # Adjust mouse position based on screen layout
                mouse.position = (
                    int(x) + current_monitor.x,
                    int(y) + current_monitor.y
                )
