...
try:
    with open("settings.txt", "r") as file:
        ...
        current_smoothing_factor = int(lines[0].strip())
        current_dead_zone_radius = int(lines[1].strip())
        current_delay = int(lines[2].strip())
except FileNotFoundError:
    current_smoothing_factor = default_smoothing_factor
    current_dead_zone_radius = default_dead_zone_radius
    current_delay = default_delay

def save_settings():
    with open("settings.txt", "w") as file:
        file.write(f"{current_smoothing_factor}\n")
        file.write(f"{current_dead_zone_radius}\n")
        file.write(f"{current_delay}\n")
...
def update_smoothing(value):
    ...
    current_smoothing_factor = int(value)
    smoothing_label.config(text=f"Smoothing Factor: {current_smoothing_factor} (Default: {default_smoothing_factor})")

def update_dead_zone(value):
    global current_dead_zone_radius
    current_dead_zone_radius = int(value)
    dead_zone_label.config(text=f"Dead Zone Radius: {current_dead_zone_radius} (Default: {default_dead_zone_radius})")

def update_delay(value):
    global current_delay
    current_delay = int(value)
    delay_label.config(text=f"Delay: {current_delay} (Default: {default_delay})")
...
smoothing_label = Label(root, text=f"Smoothing Factor: {current_smoothing_factor} (Default: {default_smoothing_factor})")
smoothing_label.pack(pady=5)
smoothing_scale = Scale(root, from_=1, to=100, orient=tk.HORIZONTAL, label="",
                        command=update_smoothing)
smoothing_scale.set(current_smoothing_factor)  # Set initial value
smoothing_scale.pack(pady=5)

# Create a Scale widget for dead zone radius
dead_zone_label = Label(root, text=f"Dead Zone Radius: {current_dead_zone_radius} (Default: {default_dead_zone_radius})")
dead_zone_label.pack(pady=5)
dead_zone_scale = Scale(root, from_=1, to=50, orient=tk.HORIZONTAL, label="",
                        command=update_dead_zone)
dead_zone_scale.set(current_dead_zone_radius)  # Set initial value
dead_zone_scale.pack(pady=5)

# Create a Scale widget for delay
delay_label = Label(root, text=f"Delay: {current_delay} (Default: {default_delay})")
delay_label.pack(pady=5)
delay_scale = Scale(root, from_=1, to=100, orient=tk.HORIZONTAL, label="",
                    command=update_delay)
delay_scale.set(current_delay)  # Set initial value
...
def update_frame():
    global last_hand_position, last_move_time  # Declare as global variables
    ret, frame = cap.read()
    ...
    if results.multi_hand_landmarks:
        ...
        if time.time() - last_move_time > current_delay:
            # Calculate hand movement speed
            hand_speed = ((index_finger[0] - last_hand_position[0]) ** 2 +
                          (index_finger[1] - last_hand_position[1]) ** 2) ** 0.5 / current_delay if last_hand_position else 0

            # Check if the movement is within the dead-zone
            if hand_speed < current_dead_zone_radius:
                hand_speed = 0
            ...
            x, y = apply_smoothing((x, y), last_hand_position, current_smoothing_factor / 100)
