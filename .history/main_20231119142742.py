...
def change_workspace():
    global current_monitor
    if current_monitor:
        current_index = get_monitors().index(current_monitor)
        next_index = (current_index + 1) % len(get_monitors())
        current_monitor = get_monitors()[next_index]
        mouse.position = (current_monitor.x, current_monitor.y)
...
# Bind the space key to change the workspace
